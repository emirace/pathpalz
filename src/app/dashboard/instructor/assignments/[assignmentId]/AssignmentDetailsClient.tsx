...
import { ChevronRight, Calendar, Users, FileText, CheckCircle, Clock, Eye, Download, Search, Filter } from "lucide-react";
import { useGetAssignmentsPerModule } from "@/query/training/student/assignment";
import { useGetAssignmentSubmissions } from "@/query/training/instructor/assignments";
...
const MOCK_SUBMISSIONS = [
  {
    ...
    submittedAt: "Oct 10, 2023, 2:30 PM",
    status: "Graded",
    score: 85,
    file: "alex_johnson_submission.pdf",
  },
  {
    id: "sub-2",
    studentName: "Maria Garcia",
    studentEmail: "m.garcia@example.com",
    submittedAt: "Oct 11, 2023, 9:15 AM",
    status: "Pending",
    score: null,
    file: "m_garcia_final_draft.docx",
  },
  {
    id: "sub-3",
    studentName: "James Smith",
    studentEmail: "james.s@example.com",
    submittedAt: "Oct 11, 2023, 4:45 PM",
    status: "Pending",
    score: null,
    file: "james_smith_algo.pdf",
  },
  {
    id: "sub-4",
    studentName: "Linda Williams",
    studentEmail: "linda.w@example.com",
    submittedAt: "Oct 12, 2023, 11:20 AM",
    status: "Graded",
    score: 92,
    file: "linda_w_project.zip",
  },
];

export default function AssignmentDetailsClient() {
  const { assignmentId } = useParams();
  const { data: assignmentsData, isLoading } = useGetAssignmentsPerModule({ moduleId: "1" });
  const { data: submissionsData, isLoading: isLoadingSubmissions } = useGetAssignmentSubmissions({ assignmentId: assignmentId as string })
  ...
  // Find assignment
  let assignment = Array.isArray(assignmentsData)
    ? assignmentsData.find((a: any) => String(a.id) === String(assignmentId))
    : null;
  ...
  if (!assignment) {
    assignment = {
      ...
      course_module: { title: "CS202: Data Structures" },
      deadline: "2023-10-12T23:59:00Z",
      statusText: "4 DAYS LEFT",
      statusColor: "text-red-600 bg-red-50 border-red-100",
      description: "Please submit your final algorithms research paper here. Ensure all references are formatted in IEEE style.",
    };
  }
  ...
  const moduleName = assignment.course_module?.title || assignment.module_title || "Unknown Module";
  ...
  const handleSaveScore = (e: React.FormEvent) => {
    e.preventDefault();
    alert(`Score ${scoreInput} and feedback saved for ${scoringSubmission.studentName}`);
    ...
    <div className="mx-auto max-w-6xl animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between mb-8 gap-4">
        <div>
          <div className="flex items-center text-sm text-gray-500 mb-2">
            ...
            <span className="font-bold text-[#00284F] truncate max-w-[200px] sm:max-w-md">
              {assignment.title}
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Total Submissions</p>
            <p className="text-2xl font-extrabold text-[#00284F]">{totalSubmissions}</p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Graded</p>
            <p className="text-2xl font-extrabold text-[#00284F]">{gradedCount}</p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-orange-100 shadow-sm flex items-center gap-4 border-l-4 border-l-orange-500">
          ...
          <div>
            ...
            <p className="text-2xl font-extrabold text-[#00284F]">{pendingCount}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 mb-8">
        <h2 className="text-lg font-bold text-[#00284F] mb-3">Assignment Description</h2>
            ...
            <tbody className="divide-y divide-gray-100">
              {submissionsData?.length === 0 ? (
                ...
                submissionsData?.map((sub) => (
                  <tr key={sub.id} className="hover:bg-gray-50/50 transition-colors group text-gray-600">
                    <td className="px-6 py-4">
                      <div>
                        <p className="font-bold text-[#00284F]">{sub.student_id}</p>
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${sub.status === "Graded" ? "bg-green-50 text-green-600 border border-green-100" : "bg-orange-50 text-orange-600 border border-orange-100"
                        }`}>
                        {sub.status}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      {sub.score !== null ? (
                        <span className="font-bold text-[#00284F]">{sub.score} / 100</span>
                      ) : (
      ...
      {scoringSubmission && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-[#00284F]/40 backdrop-blur-sm" onClick={() => setScoringSubmission(null)} />
          <div className="relative bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-lg font-bold text-[#00284F]">Grade Submission</h3>
              <button onClick={() => setScoringSubmission(null)} className="text-gray-400 hover:text-gray-600">
            ...
            <form onSubmit={handleSaveScore} className="p-6 space-y-5">
              <div>
                ...
                <p className="font-bold text-[#00284F]">{scoringSubmission.studentName}</p>
              </div>

              <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <FileText className="text-teal w-5 h-5" />
                  <span className="text-sm font-semibold text-gray-700">{scoringSubmission.file}</span>
                </div>
                <button type="button" className="text-teal hover:text-teal/80 text-xs font-bold flex items-center gap-1">
                  <Eye size={14} /> View
                </button>
              </div>

              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1.5">Score (out of 100)</label>
                <input
                  type="number"
                  required
                  min="0"
